...
- 리뷰 수정 1차 RED: 같은 명령을 권한 있는 환경에서 callback의 main-actor handoff 적용 전 실행해 exit 65, 10/11 passed, failed 1을 확인했다. background 이전 언어 load callback이 전환 뒤 후보를 갱신하지 않아야 한다는 새 회귀 테스트가 실패했으며 결과는 `/Users/macmillan/Library/Developer/Xcode/DerivedData/SYKeyboard-hgprdtyustcuukabeovkjzrtclhy/Logs/Test/Test-SYKeyboard-2026.08.13_00-49-06-+0900.xcresult`에 기록되었다. 같은-language no-op 테스트는 기존 guard 동작의 회귀 특성화이므로 이 RED 전에도 통과했다.
- 리뷰 수정 1차 진단: callback을 `@MainActor`로 handoff한 뒤의 첫 실행은 후보 load callback이 큐에 도착하기 전 `nGramSuggestionText(at:)`를 읽어 exit 65, 10/11 passed, failed 1이 되었다. 결과는 `/Users/macmillan/Library/Developer/Xcode/DerivedData/SYKeyboard-hgprdtyustcuukabeovkjzrtclhy/Logs/Test/Test-SYKeyboard-2026.08.13_00-59-48-+0900.xcresult`에 기록되었고, 실제 no-op 저장/엔진 재생성 동작은 통과했다. 테스트에서 load 뒤 `await waitForMainQueue()`로 비동기 callback 완료를 기다리도록 바로잡았다. `/Users/macmillan/Library/Developer/Xcode/DerivedData/SYKeyboard-hgprdtyustcuukabeovkjzrtclhy/Logs/Test/Test-SYKeyboard-2026.08.13_00-55-26-+0900.xcresult`는 중간의 오래된 test snapshot 컴파일 실패이므로 RED/GREEN 근거에서 제외했다.
- 리뷰 수정 1차 GREEN: 같은 명령을 권한 있는 환경에서 실행해 exit 0, 고유 11/11 passed, failed 0, skipped 0을 확인했다. destination은 iPhone 13 mini / iOS 16.0 (arm64)이며 결과는 `/Users/macmillan/Library/Developer/Xcode/DerivedData/SYKeyboard-hgprdtyustcuukabeovkjzrtclhy/Logs/Test/Test-SYKeyboard-2026.08.13_01-02-11-+0900.xcresult`에 기록되었다.
...
- GREEN: collection, switch gesture, text interaction policy 세 suite를 함께 실행해 exit 0, 고유 39/39 passed, failed 0, skipped 0을 확인했다. destination은 iPhone 13 mini / iOS 16.0 (arm64)이며 결과는 `/Users/macmillan/Library/Developer/Xcode/DerivedData/SYKeyboard-hgprdtyustcuukabeovkjzrtclhy/Logs/Test/Test-SYKeyboard-2026.08.13_01-32-14-+0900.xcresult`에 기록되었다. 실행 로그에서 Auto Layout constraint 충돌은 확인되지 않았다.
- BUILD: 같은 destination에서 `HangeulKeyboard`와 `EnglishKeyboard` scheme build를 각각 실행해 모두 exit 0을 확인했다. 최종 로그는 `/Users/macmillan/Library/Application Support/rtk/tee/1786552431_xcodebuild_build_-project_SYKeyboard_xco.log`, `/Users/macmillan/Library/Application Support/rtk/tee/1786552446_xcodebuild_build_-project_SYKeyboard_xco.log`이다.
- 리뷰 수정 1차 RED(초기 표시): collection suite를 실행해 exit 65를 확인했다. `viewDidLoad` 직후 inactive primary가 숨겨지지 않아 새 lifecycle 테스트가 실패했으며 결과는 `/Users/macmillan/Library/Developer/Xcode/DerivedData/SYKeyboard-hgprdtyustcuukabeovkjzrtclhy/Logs/Test/Test-SYKeyboard-2026.08.13_01-25-40-+0900.xcresult`에 기록되었다.
- 리뷰 수정 1차 GREEN(초기 표시): `setupUI()` 직후 `updateShowingKeyboard()`를 한 번 호출한 뒤 같은 collection suite가 exit 0으로 완료됐다. 결과는 `/Users/macmillan/Library/Developer/Xcode/DerivedData/SYKeyboard-hgprdtyustcuukabeovkjzrtclhy/Logs/Test/Test-SYKeyboard-2026.08.13_01-27-27-+0900.xcresult`에 기록되었다.
- 리뷰 수정 1차 RED(focus identity): A→nil→A와 A→nil→B production callback 테스트가 모두 실패해 exit 65를 확인했다. nil을 알리고 마지막 identity를 nil로 덮어쓴 것이 원인이며 결과는 `/Users/macmillan/Library/Developer/Xcode/DerivedData/SYKeyboard-hgprdtyustcuukabeovkjzrtclhy/Logs/Test/Test-SYKeyboard-2026.08.13_01-29-04-+0900.xcresult`에 기록되었다.
- 리뷰 수정 1차 GREEN(focus identity): non-nil identity만 비교·저장하도록 수정한 뒤 같은 collection suite가 exit 0으로 완료됐다. 결과는 `/Users/macmillan/Library/Developer/Xcode/DerivedData/SYKeyboard-hgprdtyustcuukabeovkjzrtclhy/Logs/Test/Test-SYKeyboard-2026.08.13_01-30-55-+0900.xcresult`에 기록되었다. 별도 delete/pan cancellation identity와 nil 전달 경로는 변경하지 않았다.
