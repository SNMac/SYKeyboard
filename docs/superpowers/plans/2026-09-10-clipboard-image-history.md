...
| 키보드 extension을 Xcode에 attach한 평상시 메모리와, iPad Pro 13" PNG 스크린샷(2752×2064) 복사 후 키보드를 열었을 때의 피크. 종료되면 `keyboardDecodeMemoryBudget`(32 MB)을 낮춤 | 미확인 — 이번 세션은 실기기를 사용하지 않음 |
| MacBook 스크린샷(3456×2234 PNG)을 Universal Clipboard로 복사 → 키보드에서는 항목이 생기지 않고, SY키보드 앱을 열면 관리 화면에 이미지가 나타남(예산 초과 건너뜀 → 앱 재시도) | 미확인 — 이번 세션은 실기기를 사용하지 않음 |
| 48 MP JPEG/HEIC(사진 앱에서 고해상도 촬영본) 복사 → 키보드 썸네일 생성 시간과 패널 갱신(HEIC는 24 MP 초과 시 키보드 건너뜀·앱 저장) | 미확인 — 이번 세션은 실기기를 사용하지 않음 |
