...
| 키보드 텍스트 행 탭 → 입력창 삽입 + 시스템 클립보드에 복사(다른 앱에서 복사해 둔 내용은 기록에 남음), 상세 뷰 붙여넣기도 동일 | 재확인 필요 — 탭 뒤 다른 앱에서 붙여넣기 시 그 텍스트가 나오는지, 이전 클립보드 내용이 기록 목록에 남아 있는지. 또한 "이미지도 기록" OFF에서 이미지를 복사한 뒤 텍스트 행을 탭하면 그 이미지는 덮어써져 사라지는 것이 의도된 한계임을 확인 |
| 앱 원문 시트에서 복사·고정을 누르면 시트가 닫히고 목록에 결과(맨 위 이동·고정 영역 이동)가 보임 | 재확인 필요 — 내려가는 시트에 원문이 남아 있는지(빈 시트 아님), 편집 모드에서 길게 눌러 연 시트로 고정 후 옮겨진 행의 체크 유지, VoiceOver 초점 복귀 위치 |
