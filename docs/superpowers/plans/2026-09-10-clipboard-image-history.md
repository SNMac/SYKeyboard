...
| 앱 원문 시트에서 복사·고정을 누르면 시트가 닫히고 목록에 결과(맨 위 이동·고정 영역 이동)가 보임 | 확인 — 닫힘 애니메이션 중 내용 유지. 단 `sheet(isPresented:)` 분리 방식이 이미지 행 첫 탭에 빈 시트를 만드는 회귀를 냈고, `sheet(item:)` + id 고정 래퍼로 교체(재확인 필요: 첫 탭에 사진이 보이는지, 편집 저장 후 시트 유지). 편집 모드에서 고정 후 체크 유지·VoiceOver 초점은 미확인 |
