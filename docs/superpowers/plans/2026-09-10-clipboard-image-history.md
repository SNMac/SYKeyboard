...
- 2026-09-15 삭제 알림·Analytics 로깅·사용자 속성 정리·6차 리뷰 반영 후 검증(HEAD `b4c6938d`, iPhone 13 mini / iOS 18.6): 전체 테스트 `** TEST SUCCEEDED **`, `✔ Test` 680건, `✘ Test` 0건. 확장 3개 빌드 모두 `** BUILD SUCCEEDED **`. 검증한 트리는 squash 전후 동일.
...
| 키보드 상세 뷰(길게 누르기)에서 이미지 미리보기와 "복사" 버튼 → 복원·헤더 안내 | 확인 — 동작 정상. `keyboardPreviewMaxPixelSize`는 사용자 결정으로 1200 px(3배 화면의 상세 뷰 폭 기준)로 두고, 패널이 닫힐 때 미리보기를 놓도록 함 |
| 같은 이미지 재복사 시 항목이 맨 위로 이동하고 파일이 늘지 않음, 고정 이미지는 자리 유지 | 확인 — 정상. 단, 패널을 연 채 사진 앱에서 복사하고 돌아오면 실시간 갱신 안 됨 → 1차: `onImageRecorded:` 라벨 수정으로는 해결 안 됨(호스트 복귀 시 `viewWillAppear`가 오지 않아 동기화 자체가 안 됨). 2차: `NSExtensionHostDidBecomeActive` 관찰로 동기화·패널 재조회 추가. 사용자의 원래 지적은 앱 화면이었고(별도 행), 키보드 경로도 2026-09-15 실기기 확인: 스크린샷 복사 후 돌아오면 패널에 바로 반영됨. 코드 리뷰 반영: 키보드가 화면에 없으면 읽지 않고, 목록이 바뀐 경우에만 패널을 다시 구성(제어 센터를 내렸다 올려도 상세 뷰·삭제 확인이 닫히지 않게). 실기기 확인: ② 키보드 내린 뒤 다른 앱 갔다 오기 → 권한 알림 없음(정상). ① 상세 뷰를 연 채 제어 센터 내렸다 올리기 → 상세 뷰가 닫힘. 원인 후보는 `textWillChange`의 `closeClipboardPanelIfNeeded()`(#54부터 있던 동작, develop 동일)이며 이 브랜치의 재활성화 관찰자는 목록이 같으면 아무것도 하지 않음. 사용자 확인: 패널 전체가 닫히고 자판이 나옴 → #54부터 있던 `textWillChange` 닫힘 동작이며 이 브랜치의 회귀가 아님. 바꾸려면 별도 결정 필요 |
...
| 키보드 안내를 헤더 제목 대신 하단 중앙 material 토스트로 표시(편집 모드 포함, 페이드 0.15/0.25초, 2초 유지) | 확인 — 라이트·다크 대비와 두 줄 표시 정상. 편집 모드에서 이미지 상세 "복사" 후 토스트는 미확인 |
| 키보드 텍스트 행 탭 → 입력창 삽입 + 시스템 클립보드에 복사(다른 앱에서 복사해 둔 내용은 기록에 남음), 상세 뷰 붙여넣기도 동일 | 확인 — 탭 뒤 다른 앱에서 붙여넣기와 이전 클립보드 내용의 기록 잔존 정상. "이미지도 기록" OFF에서의 덮어쓰기 한계는 미확인 |
| 앱 원문 시트에서 복사·고정을 누르면 시트가 닫히고 목록에 결과(맨 위 이동·고정 영역 이동)가 보임 | 확인 — 닫힘 애니메이션 중 내용 유지. 단 `sheet(isPresented:)` 분리 방식이 이미지 행 첫 탭에 빈 시트를 만드는 회귀를 냈고, `sheet(item:)` + id 고정 래퍼로 교체. 재확인: 첫 탭에 사진 표시, 편집 저장 후 시트 유지, 복사·고정 시 닫힘 애니메이션 중 내용 유지 모두 정상. 편집 모드에서 고정 후 체크 유지·VoiceOver 초점, 시트를 연 채 다른 앱의 키보드가 그 항목을 지우면 돌아왔을 때 "항목이 삭제되었습니다" 알림 후 시트가 닫히도록 추가. 확인 — 편집 중·비편집 모두 정상. 편집 중에는 돌아올 때 키보드가 내려가 하프 시트가 되고(iOS 동작) 확인 시 편집기 포커스 복귀로 시트가 튀던 것은 확인 시 편집 종료로는 해결되지 않았고(순서가 늦음), 시트를 먼저 닫고 목록에 띄우면 알림이 늦게 떠서, 알림이 뜨는 시점에 편집을 끝내도록 변경(재확인 필요). 저장 시 알림 경로(앱이 활성인 채로 지워지는 iPad 멀티태스킹)는 재현 어려움으로 미확인. 저장 버튼 정책(C)과 고정 상태 반영(D)은 확인 |
...
| 앱 편집 모드에서 길게 누르면 원본 시트, 탭은 선택으로만 | 확인 — 정상. 비편집 모드에서 길게 눌렀다 뗐을 때 버튼이 시트를 여는지, "상세 보기" 접근성 액션은 미확인 |
...
| 영어 기기에서 이미지 관련 패널·앱 문구가 영문 표시 | 확인 — 실기기 영어 설정에서 영문 표시됨. 문구 수정: 앱 안내의 `When 'Include Images' is off` 따옴표, 키보드 상세 뷰 제목 "상세"/"Details". 시뮬레이터는 `Embedded binary is not signed with the same certificate as the parent app`로 실행 불가(서명 설정은 이 브랜치에서 변경하지 않음) |
