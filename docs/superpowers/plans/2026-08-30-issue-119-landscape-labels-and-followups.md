...
**아래 표는 실제 기기 화면과 손 조작이 필요한 사람 관측 항목이다. 자동화 에이전트는 어떤 행도 관측하지 않았고, 아래 결과는 전부 사용자가 iPhone SE 실기기에서 직접 확인한 것이다.** OS 버전은 기록되지 않았다. 관측 근거가 없는 행은 임의로 채우지 않고 `미확인`으로 남긴다.

관측은 아래 두 후속 커밋까지 반영된 상태에서 이뤄졌다(`## 계획 이후 후속 변경` 참고). 계획 본문의 Task 3·4만 반영된 중간 상태에서는 `!#1` 겹침이 약간 남아 있었다.

| 항목 | 세로 | 가로 | 기기 |
|---|---|---|---|
| `!#1` 힌트 라벨 겹침 없음 (나랏글) | 확인 | 확인 | iPhone SE |
| `!#1` 힌트 라벨 겹침 없음 (숫자) | 확인 | 확인 | iPhone SE |
| `!#1` 힌트 라벨 겹침 없음 (천지인) | 미확인 | 미확인 | — |
| `!#1` 힌트 라벨 겹침 없음 (두벌식/쿼티) | 미확인 | 미확인 | — |
| `!#1` 힌트 라벨 겹침 없음 (기호) | 미확인 | 미확인 | — |
| `!#1` 본문 라벨 크기가 어색하지 않음 | 확인 | 확인 | iPhone SE |
| `한/A` 사선 각도 | 확인 | 확인 | iPhone SE |
| `한/A` 사선 길이 | 확인 | 확인 | iPhone SE |
| `한/A` 글자가 버튼 안에 있음 | 확인 | 확인 | iPhone SE |
| 한 손 모드에서 위 항목 유지 | 미확인 | 미확인 | — |
| 회전 시 Auto Layout 경고 없음 | 확인 | 확인 | iPhone SE |
| 글자 열 너비 슬라이더가 115에 도달 | 확인 | — | iPhone SE |
| 리뷰 팝업 전 터치 먹통 해소 | 미구현 | — | — |

관측되지 않은 행에 대한 근거:

- 천지인·두벌식/쿼티·기호 레이아웃과 한 손 모드는 사용자가 확인했다고 밝히지 않았다. 나랏글·숫자와 같은 `SwitchButton` 코드 경로를 쓰지만, 키 폭과 `insetDy`가 달라 결과가 같다고 단정하지 않는다.
- 리뷰 팝업 항목은 이번 PR에 수정이 들어가지 않았다(`## 이번 PR에서 빠진 항목` 참고).
- iPhone SE는 세로 폭 375pt로 가장 좁고 가로 높이도 가장 작아, 폭·높이 하한이 걸리는 항목에서는 가장 빡빡한 조건이다. 다만 노치/다이나믹 아일랜드 기종에서의 관측을 대체하지는 않는다.
...
| 2. `한/A` 사선 각도 | Task 4 | 세로 무변경 + 45° 하한 (사용자 결정, 최종 리뷰 후 정정) |
...
계획 실행과 최종 리뷰가 끝난 뒤, 사용자가 iPhone SE 실기기에서 확인하며 요청한 두 건이다. 계획 본문의 Task 목록에는 없다.

### `한/A` 사선이 가로에서 뭉툭해지는 문제 (commit `493da652`)

Task 4의 45° 하한은 각도를 세우려고 **가로 반길이를 세로 반길이까지 줄였다.** 그 결과 가로 획이 짧아졌다. 667pt 폭 화면에서 배경 49.6 × 32일 때 획 길이가 18.1pt로, 같은 기기 세로의 25.0pt보다 크게 짧다.

45° 하한이 걸리는 경우에만 세로 예산을 `dividerClampedHeightBoost = 1.5`배까지 쓰도록 바꿨다.

```swift
let halfWidth = size.width * dividerWidthRatio
let halfHeight = size.height * dividerHeightRatio
guard halfWidth >= halfHeight else {
    return CGSize(width: halfWidth, height: halfHeight)   // 세로: 무변경
}
let clamped = min(halfWidth, halfHeight * dividerClampedHeightBoost)
return CGSize(width: clamped, height: clamped)            // 45°, 길이 보정
```

세로는 `guard`에서 빠져나가므로 손대지 않는다. 가로 획 길이는 27.2pt가 됐다.

### 가로 `!#1` 힌트 잔여 겹침 (commit `ac66bcb6`)

Task 3이 힌트 라벨만 줄여 겹침이 약 1pt 남았다. 남은 원인은 **본문 라벨이 높이에 적응하지 않는 것**이었다. `layoutSubviews`의 크기 사다리가 너비만 읽어, 가로에서 배경 높이가 32pt인데도 16pt를 유지했다.

기존 너비 사다리를 유지한 채 낮은 키에서 한 단계 내리는 규칙을 더하고, 인라인 `if`를 `primaryLabelFontSize(forKeyWidth:keyHeight:)`로 분리해 `subLabelFontSize`와 같은 형태로 맞췄다.

| 배경 크기 | 이전 | 이후 |
|---|---|---|
| 가로 숫자 49.6 × 32 | 16 | **14** |
| 세로 49.6 × 56 | 16 | 16 |
| 세로 쿼티 44 × 39.5 (슬라이더 190) | 16 | 16 |
| 가로 자동완성 끔 44 × 43 | 16 | 16 |

문턱 `primaryLabelFullSizeKeyHeight = 36.0`은 세로 최소 배경 높이(39.5)와 가로(32) 사이라 **세로는 슬라이더 전 구간에서 무변경**이다. 자동완성을 끄면 가로 행이 47pt(`188 - 44`가 아니라 `188`을 4행이 나눔)가 되어 이 규칙도 걸리지 않는다.

두 변경 모두 사용자가 iPhone SE에서 개선을 확인했다.

## 남은 관찰 사항

이번 PR에서 진단만 하고 고치지 않은 것들이다.

- **가로에서 `한`이 `A`보다 사선에 붙어 보인다.** 간격 규칙 자체는 대칭이고(둘 다 `dividerLabelSpacing` 3pt 목표), 원인은 `clampedFrame`이 `한`을 버튼 안으로 되미는 것이다. `한`이 전각이라 shift가 크고 가로 버튼이 낮아 위쪽 `safeArea`를 먼저 넘는다. 근본 원인은 `labelFontSize(forKeyWidth:maximum:)`가 너비만 읽어 가로에서 라벨이 상한 14pt를 그대로 쓰는 것이다(세로에서는 버튼이 좁아 9.6pt로 줄어든다). `subLabelFontSize`·`primaryLabelFontSize`와 같은 형태로 높이 항을 더하면 해소된다.
- **`keyboardHeight` 190~204 구간의 세로 `한/A` 각도.** 지구본을 숨긴 기기에서 이 구간은 배경 높이가 폭의 1.1배 이하가 되어 45° 하한 경로를 탄다. 각도가 42.8° → 45°로 바뀐다. 슬라이더 최하단 근처에서만 발생한다.
- **`keyboardHeight` 190~191 구간의 세로 힌트 크기.** 쿼티계 배경 높이가 39.5pt로 `subLabelFullSizeKeyHeight`(40) 문턱을 0.5pt 차이로 넘어 힌트가 8.0 → 7.9pt가 된다. 힌트 규칙도 `primaryLabelFontSize`처럼 이산형으로 바꾸면 없어진다.
