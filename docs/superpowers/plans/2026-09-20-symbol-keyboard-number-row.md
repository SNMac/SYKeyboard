...
미확인으로 남긴다. 2026-09-21 기준이며 URL·이메일 자판 통일까지 반영했다.
...
- [ ] **기호 자판의 숫자 키를 눌러 1~0이 실제로 입력된다.** 눌린 표시·소리·진동도 난다.
      ...
      일어나지 않던 상태였다
- [ ] URL·이메일 입력란에서 기호 자판 배열이 기본 자판과 **같다.** 1페이지 첫 줄이
      `- / : ; ( ) ₩ & @ ”`, 둘째 줄이 `[ ] { } # % ^ * + =`
- [ ] URL·이메일 입력란에서 `⇧`가 **보이고**, 눌러 2페이지로 넘어간다
...
- [ ] **URL 칸을 쓰다가 twitter 칸으로 가면** 두벌식 아랫줄에 `.` `/` `.com`이 사라지고
      `@` `#`이 나온다. 스페이스바가 정상 너비다 (`cbad077f`)
- [ ] 반대로 twitter → URL, twitter → 이메일, twitter → 일반도 각각 맞는 키만 나온다
- [ ] 영어 자판에서도 같은 전환이 정상이다 (두벌식을 영어에 맞춰 고쳤다)
- [ ] 한영 키보드를 띄울 때 Xcode 콘솔에 `Unable to simultaneously satisfy constraints`가
      찍히지 않는다 (`f23e5575`로 되돌린 뒤 사라진 것을 확인했다)

### 한영 자동 전환 (`553d133a`)

연락처 앱의 새 연락처 화면이 확인하기 좋다. 한 화면에 타입이 다른 칸이 모여 있다.

- [ ] 이름 칸(일반)에서 한글로 맞춘 뒤 **이메일 칸을 탭하면 영어로 바뀐다**
- [ ] 다시 이름 칸을 탭하면 **마지막 언어(한글)로 돌아온다**
- [ ] 같은 칸 안에서 수동으로 한영 전환한 선택이 계속 유지된다. 커서를 옮기거나 타이핑해도
      되돌아가지 않는다
- [ ] 한글 조합 중(예: "한"까지 친 상태)에 다른 칸으로 옮겨도 글자가 깨지거나 남지 않는다
- [ ] 자동 전환이 일어나는 순간 잘못된 언어의 자판이 한 프레임이라도 보이지 않는다.
      `updateKeyboardType()`이 이전 언어로 한 번 정한 뒤 정정하는 구조라 짚어둔다
- [ ] 전화 칸을 탭하면 숫자 자판이 뜬다

### 그 밖

- [ ] URL·이메일 자판의 셋째 줄 키가 좌우 가운데에 놓인다
- [ ] 설정 화면 미리보기가 키보드 높이 최대에서 잘리지 않는다
