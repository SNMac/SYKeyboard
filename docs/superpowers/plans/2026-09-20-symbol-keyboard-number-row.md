...
자동 테스트와 빌드로는 확인할 수 없다. 실제 입력 앱에서 확인하고, 확인하지 못한 항목은
미확인으로 남긴다.

**2026-09-21 사용자가 실기기에서 전 항목을 확인했고 모두 정상이었다.** 아래 체크는
사용자 보고에 따른 것이며 내가 직접 관찰한 것이 아니다. 2026-09-21 기준이며 URL·이메일 자판 통일까지 반영했다.

### 반드시 확인 (이번에 고친 결함)

- [x] **기호 자판의 숫자 키를 눌러 1~0이 실제로 입력된다.** 눌린 표시·소리·진동도 난다.
      브랜치 전체 리뷰가 잡은 Critical이다. 숫자 행이 그려지지만 눌러도 아무 일도
      일어나지 않던 상태였다
- [x] URL·이메일 입력란에서 기호 자판 배열이 기본 자판과 **같다.** 1페이지 첫 줄이
      `- / : ; ( ) ₩ & @ ”`, 둘째 줄이 `[ ] { } # % ^ * + =`
- [x] URL·이메일 입력란에서 `⇧`가 **보이고**, 눌러 2페이지로 넘어간다
- [x] URL·이메일 자판의 삭제 버튼 폭이 기본 자판과 같다

### 숫자 행

- [x] 세로에서 기호 자판을 열면 숫자 행이 두벌식·쿼티와 같은 자리, 같은 높이로 보인다
- [x] `⇧`를 누르면 첫 줄이 `_ \ | ~ < > $ £ ¥ •`, 둘째 줄이 `※ ☆ ★ ○ ● □ ■ △ ▲ ♡`다
- [x] 도형 열 개가 모두 제 모양으로 보이고, `♡`가 빨간 이모지로 바뀌지 않는다
- [x] 자판을 전환해도(한영·기호·숫자) 전체 프레임 높이가 바뀌지 않는다
- [x] 가로로 돌려도 숫자 행이 그대로 있고 기호 배열도 같다. 숫자 행만 얇아진다(35pt)
- [x] 숫자 행 설정을 끄면 기호 자판이 예전 배열(첫 줄이 숫자)로 돌아온다
- [x] 나랏글·천지인만 쓰는 한글 extension에서는 기호 자판에도 숫자 행이 없다
- [x] 자판 선택 오버레이와 한 손 모드 오버레이가 숫자 행을 가리지 않는다

### 스페이스 옆 키 (애플과 다르게 남기기로 한 부분)

- [x] URL 입력란 아래에 `.` `/` `.com`이 있다
- [x] 이메일 입력란 아래에 `@`와 `.`이 있다
- [x] 웹 검색 입력란 아래에 `.`이 있다
- [x] 일반 입력란에는 스페이스만 있다

### 실기기 테스트 중 발견해 고친 버그

- [x] **URL 칸을 쓰다가 twitter 칸으로 가면** 두벌식 아랫줄에 `.` `/` `.com`이 사라지고
      `@` `#`이 나온다. 스페이스바가 정상 너비다 (`cbad077f`)
- [x] 반대로 twitter → URL, twitter → 이메일, twitter → 일반도 각각 맞는 키만 나온다
- [x] 영어 자판에서도 같은 전환이 정상이다 (두벌식을 영어에 맞춰 고쳤다)
- [x] 한영 키보드를 띄울 때 Xcode 콘솔에 `Unable to simultaneously satisfy constraints`가
      찍히지 않는다 — 2026-09-21 실기기 확인

### 한영 자동 전환 (`553d133a`)

연락처 앱의 새 연락처 화면이 확인하기 좋다. 한 화면에 타입이 다른 칸이 모여 있다.

- [x] 이름 칸(일반)에서 한글로 맞춘 뒤 **이메일 칸을 탭하면 영어로 바뀐다** — 확인
- [x] 다시 이름 칸을 탭하면 **마지막으로 직접 고른 언어**로 돌아온다 — 확인
- [x] 같은 칸 안에서 수동으로 한영 전환한 선택이 계속 유지된다 — 확인.
      필드마다 언어를 기억하는 것이 아니라, trait이 안 바뀌면 판정을 다시 하지 않아서
      직전 언어가 남는 것이다. 마지막 수동 선택은 전역 값 하나로만 저장된다
- [x] 한글 조합 중(예: "한"까지 친 상태)에 다른 칸으로 옮겨도 글자가 깨지거나 남지 않는다 — 확인
- [x] 자동 전환이 일어나는 순간 잘못된 언어의 자판이 한 프레임이라도 보이지 않는다 — 확인.
      `updateKeyboardType()`이 이전 언어로 한 번 정한 뒤 정정하는 구조라 짚어뒀던 항목이다
- [x] 전화 칸을 탭하면 숫자 자판이 뜬다

### 그 밖

- [x] URL·이메일 자판의 셋째 줄 키가 좌우 가운데에 놓인다
- [x] 설정 화면 미리보기가 키보드 높이 최대에서 잘리지 않는다

### 시뮬레이터에서 이미 확인한 것 (다시 볼 필요 없음)

iPhone 13 mini / iOS 18.6 시뮬레이터에서 2026-09-21에 확인했다.

- URL 입력란의 기호 자판이 기본 배열로 나온다. 1·2페이지 모두 확인
- URL 입력란에서 영어 자판이 먼저 뜬다
- 스페이스 옆 `.` `/` `.com`이 그대로 있다

숫자 행을 켠 상태는 확인하지 못했다. 설정은 켰으나 그 뒤 화면 이동이 되지 않았다.
