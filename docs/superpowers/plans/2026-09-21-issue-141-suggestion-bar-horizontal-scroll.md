...
실기기 / iOS 27에서 확인했다. 아래 결과는 모두 `fa8d450d` 기준이다.
(기준 시뮬레이터 `iPhone 13 mini / iOS 18.6`은 자동 테스트용이고, `UIScrollEdgeEffect`처럼
iOS 26+에서만 나타나는 증상은 실기기에서만 관찰된다.)

### 터치·선택

- [x] 짧은 탭으로 후보가 선택되고, 끌면 스크롤된다
- [x] 후보에 손을 댄 즉시 하이라이트가 켜진다(`delaysContentTouches = false`)
- [x] 후보가 넘치지 않으면(수식 3칸 포함) 끌어서 고르기가 동작한다
      — 결정 4번으로 되살린 동작. 처음에는 탭 전용으로 통일했다가 사용자 확인 후 되돌렸다
- [x] 터치·드래그 중에는 preview 하이라이트가 드러나지 않고, 손을 떼면 되살아난다 — 결정 5번
- [x] 후보와 undo/redo를 두 손가락으로 동시에 눌러도 액션이 한 번만 발생하고,
      먼저 누른 손가락을 떼기 전에 자판 입력이 다시 켜지지 않는다
      (Task 1 리뷰의 멀티터치 가드 수정. `UITouch`를 합성할 수 없어 자동 테스트로 덮이지 않는다)
- [x] 클립보드·undo/redo 버튼이 스크롤과 무관하게 동작한다
      (**후보가 10개 차 있고 스크롤하지 않은 기본 상태**에서 undo/redo 탭·길게 누르기,
      그 다음 **오른쪽으로 절반쯤 스크롤한 상태**에서 클립보드 버튼 탭.
      최종 리뷰가 잡은 Critical이 났던 자리이고, 후보 3개 상태에서는 정상으로 보여 그냥 지나간다)

### #139 회귀

- [x] 스크롤 중에는 삭제 확인 오버레이가 뜨지 않는다
- [x] 제자리에서 0.5초 누르면 삭제 확인 오버레이가 뜬다

### 표시

- [x] 후보 글자가 뭉개지지 않는다 (iOS 26+ 기본 `UIScrollEdgeEffect`를 끈 뒤 선명함 확인) — 결정 1번
- [x] 후보가 3개보다 적어도 후보 영역이 3칸으로 보인다 — **회귀로 확인되어 복구함.**
      가변 개수로 바꾸면서 divider를 `후보 수 - 1`로 묶은 것이 `develop`의 동작을 깼다.
      `develop`에서는 `leftDivider`/`rightDivider`만 생성 시 `isHidden = true`가 없어 항상
      보였고 `updateSuggestions`도 divider를 건드리지 않아, 후보가 1개여도 바가 3등분으로
      보였다. divider를 열 격자 기준으로 되돌렸다(`dividerCount(forSuggestionCount:)`)
- [x] 텍스트 대치 항목이 항상 후보 1번 칸에 온다 — 결정 6번
      (현재 엔진 구현에서 추가 코드 없이 보장된다. 실기기 확인 완료)
- [x] 후보 10개 상태에서 스페이스로 대치될 preview 하이라이트가 화면 밖에 있을 때
      대치가 예고 없이 일어나는 인상을 주지 않는다 (자동 스크롤하지 않는 것이 결정이다)
- [x] ~~라이트/다크 모드에서 가장자리 페이드가 의도대로 보인다~~ — 페이드 제거로 해당 없음
- [x] ~~스크롤하는 동안 페이드가 지연 없이 따라온다~~ — 페이드 제거로 해당 없음
- [x] ~~반투명 키보드 배경에서도 페이드가 어색하지 않다~~ — 페이드 제거로 해당 없음

### 레이아웃

- [x] 수식 후보 3칸이 스크롤되지 않는다
- [x] 클립보드 패널을 열고 닫아 후보 영역 폭이 바뀌어도 버튼 폭이 다시 맞는다
- [x] 키보드 높이 조절과 한손 모드 전환 중 후보 버튼 폭이 한 프레임 어긋나지 않는다
...
iPhone / iOS 27에서 후보 글자가 뭉개져 보인다는 보고로 확인한 결과와 그에 따른 결정이다.

1. **원인은 iOS 26부터 `UIScrollView`에 기본으로 켜지는 `UIScrollEdgeEffect`였다.**
   `isHidden` 기본값이 `false`라 스크롤 뷰를 만들기만 해도 적용된다. 후보 바처럼 짧고 넓은
   스크롤 뷰에서는 좌우 효과가 후보 대부분을 덮는다. `.automatic`/`.hard`/`.soft` 어느
   스타일로도 달라지지 않아 네 방향 모두 `isHidden = true`로 껐다. iOS 18.6에는 이 API가
   없어 증상도 없었다.

2. **spec 「확정된 결정」 5번(가장자리 페이드로 더 볼 후보가 있음을 알린다)을 철회했다.**
   사용자 결정이다. 정위치(`contentOffset` 0)에서는 다음 후보의 글자가 경계에서 약 39pt
   안쪽에 중앙 정렬되어 있어 16pt 페이드 영역 안에 흐릴 대상이 아예 없다. 결과적으로
   스크롤 중에만 보이는데, 그때는 사용자가 이미 스크롤하고 있어 정보 가치가 없다.
   `UIScrollEdgeEffect`에는 색상·폭 API가 없어(`isHidden`과 `style`뿐) 공식 API로 대체할
   방법도 없다. `CAGradientLayer` mask는 매 프레임 offscreen 렌더링을 강제하므로 키보드
   입력 경로에서 그냥 비용이다. 따라서 `edgeFadeLayer`·`updateEdgeFade()`·`edgeFadeWidth`·
   `SuggestionScrollFadePolicy`와 그 테스트 5개를 제거했다.

3. **스크롤 가능 여부를 알리는 표시는 현재 없다.** 갤럭시 키보드도 표시하지 않는다는 점을
   확인했다. 발견 가능성이 실제 문제로 드러나면 그때 최소한의 장치를 더한다.

4. **끌어서 고르기를 후보가 넘치지 않을 때는 되살렸다.** 사용자 결정이다. spec 「확정된 결정」
   3번을 "넘칠 때만 탭 전용"으로 좁힌다. 넘치지 않으면 `UIScrollView`의 pan이 시작되지 않아
   끄는 동작이 스크롤로 해석될 일이 없으므로, 기존 동작을 없앨 이유가 없다. 수식 3칸도 여기에
   해당한다.

5. **터치 중에는 preview 하이라이트를 숨긴다.** 탭 전용에서 시작한 후보를 벗어나면 눌린
   하이라이트가 사라지는데, `SuggestionHighlightPolicy`가 touched → action → preview 순으로
   보기 때문에 그 순간 preview가 드러나 끌고 있는 동안 가운데 칸이 선택된 것처럼 보였다.
   정책은 그대로 두고 입력값만 막았다.

6. **텍스트 대치 항목은 이미 후보 1번 칸에 온다 — 추가 코드 없이 보장된다.**
   사용자가 요구사항으로 제시했고, 한때 `prioritizedLexiconResults(_:baseText:)`로 명시 보장을
   넣었다가(`7a12a22b`) 코드 리뷰에서 no-op임이 드러나 되돌렸다.
   `LexiconPredictiveTextEngine.textReplacementEntries(matching:)`는 소문자 키 **정확 일치**
   딕셔너리 조회라 같은 키의 항목은 `userInput` 길이가 모두 같고, `max(by:)`는 동수에서 첫
   원소를 돌려준다(Swift로 직접 확인). `suggestions(for:)`는 같은 배열을 순서 그대로
   매핑하므로 match는 **언제나 `results[0]`**이다. 여기에 `mergeSuggestions`가 lexicon을
   TextChecker보다 먼저 넣으므로 후보 배열 0번 = 바 1번 칸이 된다.
   되돌린 이유: 실기기에서 밀리는 것을 관찰한 적이 없고(요구사항 진술이었다), 남기면 키
...
`contentSize.width`가 뷰포트를 넘지 않고, 스크롤이 사라진다. 가장자리 페이드는 실기기 확인에서
이미 제거했으므로(위 「실기기 확인에서 바뀐 결정」 2번) 되돌릴 대상이 아니다.
