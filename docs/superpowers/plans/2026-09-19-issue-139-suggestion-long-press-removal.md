...
- 1차 확인 시점의 `removalLongPressDuration`은 `0.7`초였다. 확인 후 "0.7초는 너무 길다"는 사용자 판단으로 `0.5`초로 조정했다(별도 커밋).
- `0.5`초로 조정한 뒤 **누르는 시간에 직접 의존하는 항목**(길게 눌렀을 때 오버레이 표시, 드래그 중 오버레이 미표시)을 사용자가 다시 확인했고 이상 없음.
