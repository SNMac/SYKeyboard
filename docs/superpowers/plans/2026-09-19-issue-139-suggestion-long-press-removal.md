...
- [x] **Step 1: 한글 키보드**
  - [x] 짧은 탭 선택이 기존과 같음
  - [x] 드래그 선택이 기존과 같고, 드래그 중 오버레이가 뜨지 않음
  - [x] NGram 후보를 길게 누르면 오버레이 표시·햅틱, 손을 떼도 후보가 입력되지 않음
  - [x] 삭제 후 해당 단어가 다시 나오지 않음 (키보드를 닫았다 열어도)
  - [x] 취소 시 아무것도 바뀌지 않음
  - [x] 오버레이가 떠 있는 동안 키 입력이 막힘
  - [x] 텍스트 대치 후보를 길게 누르고 떼면 기존처럼 입력됨
- [x] **Step 2: 영어 키보드**
  - [x] 시스템 사전 후보를 길게 누르고 떼면 기존처럼 입력됨
  - [x] NGram 후보 삭제가 한글 키보드와 같게 동작
- [x] **Step 3: 한영 통합 키보드**
  - [x] 한국어 모드에서 지운 단어가 영어 모드 데이터에 영향 없음
- [x] **Step 4: 클립보드 회귀**
  - [x] 고정 항목이 섞인 삭제 확인의 모양·문구·동작이 기존과 같음
- [x] **Step 5: 결과 기록 후 커밋**

**결과 (2026-09-21)**

- 사용자가 실제 입력 앱에서 Step 1~4의 모든 항목을 직접 확인했고 이상 없음. 확인 환경은 사용자 로컬 환경이며 기기·OS 버전은 기록하지 않았다.
- 확인 시점의 `removalLongPressDuration`은 `0.7`초였다. 확인 후 "0.7초는 너무 길다"는 사용자 판단으로 `0.5`초로 조정했다(별도 커밋).
- 따라서 **누르는 시간에 직접 의존하는 두 항목**(길게 눌렀을 때 오버레이 표시, 드래그 중 오버레이 미표시)은 `0.5`초에서 다시 확인하지 않았다. 드래그는 다른 후보로 넘어가는 순간 타이머를 취소하므로 회귀 가능성은 낮지만, 같은 후보 위에서 천천히 끄는 경우는 값이 짧아진 만큼 영향을 받을 수 있다.
