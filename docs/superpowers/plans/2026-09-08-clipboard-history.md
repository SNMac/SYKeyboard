...
| 키보드 편집 모드에서 "선택 해제" ↔ "전체 선택" 전환 시 글자가 잘리지 않음 | 미확인 (iOS 26+에서 "전체 선택 해제"가 잘리던 문제. 폭 고정 대신 같은 글자 수의 "선택 해제"로 변경) |
...
| 편집 중복 병합에서 고정→미고정 텍스트는 고정 맨 위로, 미고정→고정 텍스트는 그 고정 항목이 맨 위로. `+`로 이미 고정된 텍스트를 추가하면 그 항목이 고정 맨 위로 | 미확인 |
| 앱 삭제 확인 시트가 스와이프한 행(스와이프 삭제)과 하단 바 삭제 버튼(편집 모드)에서 뜸(iOS 26+), iOS 16은 하단 시트 | 미확인 |
