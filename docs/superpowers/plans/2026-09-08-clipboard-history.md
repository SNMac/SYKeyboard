...
| 키보드 편집 모드에서 "전체 선택 해제" → "전체 선택"으로 돌아올 때 버튼이 줄며 글자가 잘리지 않음 | 미확인 (iOS 26+ 실기기에서 재현, 폭 고정으로 수정) |
| 고정·미고정이 섞인 삭제의 확인 문구가 "항목 n개를 삭제할까요?" + 고정 개수 설명(키보드·앱) | 미확인 |
| 앱 원문 편집에서 다른 항목과 같은 내용으로 저장하면 편집 항목이 사라지고 기존 항목이 맨 위로 옴(고정이면 그대로), 시트는 그 항목을 보여줌 | 미확인 |
| 앱 목록 행을 탭해 시트를 연 뒤 눌린 표시가 남지 않음 | 미확인 (간헐 재현되던 문제를 탭 제스처로 변경) |
