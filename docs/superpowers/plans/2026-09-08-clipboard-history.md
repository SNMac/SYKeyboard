...
| 앱 관리 화면 편집 모드 "n개 고정"/"n개 고정 해제"(대상 없음·한도 초과 시 비활성) | 미확인 |
| 키보드 패널 trailing swipe가 `trash.fill` 아이콘으로 표시 | 미확인 |
| 패널 문자열을 `SYKeyboardAssets` 카탈로그로 옮긴 뒤 영어 기기에서 세 키보드 패널·전체 접근 허용 안내 오버레이가 영문 표시 | 미확인 (세 extension scheme BUILD SUCCEEDED, appex 안 `SYKeyboardAssets_SYKeyboardAssets.bundle/en.lproj/Localizable.strings`에 18개 포함 확인) |
| 전체 접근 허용 안내 오버레이가 Base로 옮긴 뒤에도 세 키보드에서 뜨고, 닫기·시스템 설정 이동이 동작 | 미확인 |
| 앱 미리보기 키보드에 전체 접근 허용 안내가 뜨지 않음 | 미확인 |
