...
| 항목 | 결과 |
| --- | --- |
| 복사 → 키보드 재등장 시 기록 반영 | 미확인 |
| 복사 → 텍스트 필드 전환 시 기록 반영 | 미확인 |
| 복사 → 클립보드 버튼 탭 시 기록 반영 | 미확인 |
| iOS 16 붙여넣기 권한 배너 표시, "다른 앱에서 붙여넣기" 허용 후 사라짐 | 미확인 |
| 한글 조합 중 붙여넣기 후 다음 자모가 새 글자로 시작 | 미확인 |
| 붙여넣기가 undo 1단위 | 미확인 |
| 편집 모드 체크마크와 모든 행 들여쓰기 | 미확인 |
| 편집 모드에서 스와이프 삭제가 막힘 | 미확인 |
| 평소 모드 trailing swipe 삭제 | 미확인 |
| 길게 누르기 상세 뷰 스크롤·붙여넣기·닫기 | 미확인 |
| Full Access OFF에서 안내 상태 | 미확인 |
| 자동완성 OFF에서 클립보드 버튼 미표시 | 미확인 |
| 한 손 모드·가로 모드에서 패널 폭·높이 | 미확인 |
| 클립보드 버튼 아이콘이 패널 열림/닫힘에 따라 바뀜 | 미확인 |
| 비밀번호 관리자에서 복사한 항목(org.nspasteboard.ConcealedType)이 기록되지 않음 | 미확인 |
| leading swipe로 고정/해제, 고정 행 pin 아이콘, 고정 항목이 맨 위 고정 순 | 미확인 |
| 고정 20개가 차면 미고정 행에 leading swipe 없음 | 미확인 |
| 상세 뷰 고정/해제 버튼 후 목록 갱신 | 미확인 |
| 앱 설정 '클립보드 기록 관리' 화면: 목록·스와이프·편집 모드가 키보드 패널과 같음 | 미확인 |
| 앱에서 직접 추가한 항목이 고정으로 맨 위에 오고 키보드 패널에 바로 반영 | 미확인 |
