...
| 영어 기기에서 패널 문자열(Full Access 안내 등)이 영문으로 표시 | 77df2e5f 빌드에서 세 키보드 모두 한국어로만 표시됨(실기기). 원인: Core가 정적 라이브러리라 Core 카탈로그가 extension 번들에 안 들어감. Common·앱 카탈로그로 옮긴 뒤 재확인 필요 |
| iOS 16 앱 관리 화면 편집 모드에서 하단 바(전체 선택·n개 삭제) 표시 | 77df2e5f 빌드에서 iOS 16 실기기는 하단 바가 안 뜸(iOS 27은 정상). bottomBar 항목을 항상 두고 `.toolbar(_:for:)`로 표시만 토글하도록 수정, 재확인 필요 |
