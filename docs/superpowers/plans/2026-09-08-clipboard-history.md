...
| 평소 모드 trailing swipe 삭제 | 삭제 확인. 행 삭제 애니메이션은 7c8a02a6 이후 재확인 필요 |
...
| 비밀번호 관리자에서 복사한 항목(org.nspasteboard.ConcealedType)이 기록되지 않음 | iOS '암호' 앱은 public.utf8-plain-text만 넣어 기록됨(실기기 로그 확인, 구분 불가). 서드파티 관리자(1Password 등)는 미확인 |
...
| 앱 설정 '클립보드 기록 관리' 화면: 목록·스와이프·편집 모드가 키보드 패널과 같음 | 미확인 (00713ed1·f840d2ab 수정 후 재확인 필요: 편집→선택→n개 삭제, 원문 하프 시트) |
