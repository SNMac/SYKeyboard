...
| 키보드 상세 뷰·앱 원문 시트에서 항목 전체가 URL이면 본문이 파란 밑줄 링크로 보이고 탭하면 브라우저가 열림. URL이 아니면 일반 텍스트 | 미확인 |
| 앱 원문 시트 왼쪽 상단 공유 버튼으로 공유 시트가 뜸 | 미확인 |
| 앱 `List`에서 고정 항목 스와이프 삭제 시 확인 시트가 뜨는 동안 행이 접혔다 돌아옴 | 재현됨 (2026-09-09, iOS 16·27 실기기). destructive role의 선행 애니메이션. `UITableView` 전환(6095f645 시점)은 iOS 27에서 지워지는 행이 카드 위로 올라가는 문제가 있어 되돌리고 `List` 유지. 알려진 제한 |
| 앱 원문 시트의 고정/해제 버튼이 즉시 시트와 목록에 반영되고, 복사하면 시트 뒤 목록에서 항목이 맨 위로 올라감(고정 항목은 그대로). 편집 모드 편집기에 흰 사각형 배경이 없음 | 미확인 |
| 키보드 패널 편집 모드의 "완료"만 semibold("n개 삭제"는 regular로 되돌림) | 미확인 |
| 키보드 패널에서 행을 스와이프 삭제해도 편집 모드로 바뀌지 않음 | 미확인 (실기기에서 재현됐던 문제를 `isItemEditing` 플래그로 수정) |
| 키보드에서 행 탭·상세 뷰 붙여넣기로 쓴 미고정 항목이 다음에 패널을 열면 맨 위(고정 아래)에 옴. 고정 항목은 자리 유지 | 미확인 |
| 앱에서 고정 항목이 포함된 삭제(스와이프·편집 모드)는 알림으로 확인 후 삭제되고, 취소하면 유지 | 미확인 |
| 키보드 패널에서 고정 항목이 포함된 삭제(스와이프·편집 모드)는 패널 안 확인 뷰가 뜨고, 삭제 시 지워지며 취소·패널 닫기 시 유지 | 미확인 |
| 키보드 고정 행 스와이프 삭제 → 확인 뷰 → 취소 시 행이 제자리에 남음(completion false) | 미확인 (2차 리뷰 반영) |
| 키보드 상세 뷰에서 URL 항목의 빈 여백 탭은 브라우저를 열지 않고 글자 영역 탭만 엶 | 미확인 (2차 리뷰 반영) |
| 키보드에서 스와이프를 연 채 패널을 닫고 다시 열면 스와이프가 닫혀 있고, 스와이프 연 채 "선택"을 눌러도 헤더와 테이블이 함께 편집 모드 | 미확인 (2차 리뷰 반영) |
| 앱 삭제 확인 시트가 닫히는 동안 제목이 "0개"로 바뀌지 않음, 자동완성 설정 화면의 토글 조작 시 관리 화면 저장소 읽기가 일어나지 않음(LazyView) | 미확인 (2차 리뷰 Minor 반영) |
| 앱 원문 시트 "편집"으로 내용을 고쳐 저장하면 같은 자리에 반영되고, 빈 값·중복·원문과 같으면 저장 비활성 | 미확인 |
...
| 앱 편집 모드에서 선택이 있으면 제목이 "n개 선택", 하단 바가 `checklist.checked`/`checklist.unchecked`·`pin`/`pin.slash`·`trash` 아이콘으로 표시 | 미확인 |
