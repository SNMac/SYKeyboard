...
| 화면 | iOS 27 화면 동일 | iOS 18.6 화면 동일 | 뒤로 스와이프 동일 (iOS 27) | 취소·저장 (iOS 27) |
|---|---|---|---|---|
| 키보드 높이 | 픽셀 동일 | 동일 (차이는 클립보드 아이콘·세그먼트 눌림 표시뿐) | 동일 (안 됨) | 정상 |
| 글자 열 너비 | 픽셀 동일 | 동일 (차이는 클립보드 아이콘뿐) | 동일 (안 됨) | 정상 |
| 한 손 키보드 너비 | 픽셀 동일 | 동일 (차이는 클립보드 아이콘뿐) | 동일 (안 됨) | 정상 |
| 길게 누르기 입력 | 픽셀 동일 | 픽셀 동일 | 동일 (됨) | 해당 없음 |
| 커서 이동 | 픽셀 동일 | 동일 (차이는 홈 인디케이터 줄뿐) | 동일 (됨) | 해당 없음 |

`VStack(spacing: 0)` 조정 여부: 불필요. 모든 화면이 기본 `VStack`으로 변경 전과 같다.

**실행 결과 (2026-09-23, 빌드 `e7faa99c`)**

- 비교는 캡처를 `imgdiff`(scratchpad `issue-151/imgdiff.swift`, CoreGraphics로 RGB 차이 합이 24를 넘는 픽셀을 센다)로 했다. `imgdiff <before>.png <after>.png` → `identical` 또는 차이 픽셀 수와 영역을 출력한다.
- iOS 18.6의 차이는 모두 화면 구조와 무관한 상태 차이다. 변경 전 캡처 **뒤에** '클립보드 기록'을 켜서 미리보기 키보드 툴바에 클립보드 아이콘이 생겼고(Task 1 결과 참고), 미리보기 언어는 '한 손 키보드 너비' 화면이 열릴 때 시스템 언어로 바꾸는 저장값이라 키보드 높이 비교 때 '한글 키보드' 세그먼트를 눌러 맞췄다. 캡처를 열어 제목·툴바·숫자·슬라이더·설명·미리보기 위치가 같음을 확인했다.
- 취소·저장: `savecancel.py`(scratchpad `issue-151/`)로 끝까지 민 뒤 '취소' → 재진입 값이 기본값, 끝까지 민 뒤 '저장' → 재진입 값이 최대값, '리셋'+'저장'으로 기본값 복원까지 세 화면 모두 `OK`.
- 길게 누르기·커서 이동: 테스트 필드를 탭해 키보드를 올린 뒤 슬라이더를 움직이면 키보드가 내려갔다(`hideKeyboard()` 동작 유지).
- iOS 18.6은 저장값(키보드 높이 150)을 바꾸지 않으려고 취소·저장을 돌리지 않았다. 컨테이너만 바뀐 변경이라 iOS 27 확인으로 갈음한다. 뒤로 스와이프는 Task 1대로 18.6에서 측정 불가.
...
- [x] **Step 2: 편집 모드·하단 바·시트 동작을 두 OS에서 확인한다**

항목이 2개 이상 있어야 한다. 없으면 '추가'(+)로 텍스트 고정 항목을 두 개 만든다. 두 시뮬레이터에서 아래를 확인하고 캡처를 `after/`에 남긴다.

| 확인 | iOS 27 | iOS 18.6 |
|---|---|---|
| 진입 시 제목 '클립보드 기록', 오른쪽 '+'·'선택' 표시, 하단 바 없음 (before 캡처와 같음) | 픽셀 동일 | 픽셀 동일 |
| '선택' → 하단 바 표시, 버튼이 '완료'로 바뀜 | 정상 | 정상 |
| 행 선택 시 제목이 'n개 선택'으로 바뀜 | 정상 (편집 캡처 픽셀 동일) | 정상 (편집 캡처 픽셀 동일) |
| 하단 바 '전체 선택'/'선택 해제' 토글 | 정상 | 측정 불가 (아래) |
| 하단 바 고정 버튼으로 고정/해제 후 편집 모드 종료 | 정상 | 측정 불가 (아래) |
| 하단 바 삭제 → 확인 시트가 삭제 버튼 근처에 뜸 → 취소 | 정상 (고정 항목 포함 시) | 측정 불가 (아래) |
| 행 왼쪽 스와이프(고정)·오른쪽 스와이프(삭제 확인 시트) | 정상 (고정 행 삭제 시 행 근처 확인 시트) | 스와이프 버튼 노출 정상, 시트는 미확인 |
| 행 탭 → 상세 시트, 제목·툴바 표시, 닫기 | 정상 | 정상 |
| '+' → '고정 항목 추가' 시트, 취소·저장 | 표시·취소 정상, 저장은 안 함 | 표시·취소 정상, 저장은 안 함 |
| 편집 모드 중 뒤로 가기 → 상위 화면에 빈 하단 바가 남지 않음 (Review Focus 5) | 남지 않음 | 남지 않음 |
| 뒤로 스와이프 동작이 Task 1 기준표와 같음 | 동일 (됨) | 측정 불가 |

**실행 결과 (2026-09-23, 빌드 `1b384989`)**

- 계획의 기대와 다른 점: **고정되지 않은 항목은 확인 시트 없이 바로 삭제된다.** `requestRemove(_:source:)`가 고정 항목이 섞였을 때만 `pendingDeletion`을 세우는 원래 설계다(이번 diff와 무관). 이를 모르고 iOS 27 시뮬레이터에서 고정되지 않은 테스트 항목 '클립보드' 하나를 지웠다. 확인 시트는 항목을 고정한 뒤 하단 바 삭제·행 스와이프 삭제 모두에서 확인하고 바깥을 눌러 취소했다. 고정 상태는 모두 원래대로 풀었다.
- 상위 화면: 클립보드 기록 관리는 메인 화면에서 push되므로 편집 중 뒤로 가면 메인 화면으로 돌아간다. 두 OS 모두 하단 바가 남지 않았다.
- **iOS 18.6 하단 바는 측정 불가**: 전체 선택 버튼을 `idb ui tap`(duration 0.1·0.2, 좌표 3곳)으로 눌러도 화면이 바뀌지 않았고, **변경 전 앱(`before.app`)에서도 똑같이 반응하지 않았다.** 이번 변경의 회귀가 아니라 `idb`가 iOS 18.6 SwiftUI 하단 툴바에 탭을 전달하지 못하는 것으로 본다. 하단 바 표시 여부와 편집 캡처는 픽셀 동일로 확인했다.
- iOS 18.6은 `idb` 탭이 자주 무시돼 화면 진입을 매번 확인했다. 한 번은 진입에 실패한 채 누른 탭이 메인 화면 '학습 데이터 초기화'에 닿아 확인 알림이 떴고, '취소'를 눌러 아무것도 지워지지 않았다.
- '저장'(고정 항목 추가)은 시뮬레이터 기록을 늘리지 않으려고 누르지 않았다. 시트는 자체 `NavigationStack`을 그대로 두어 이번 변경의 영향이 없다.
- iOS 16 화면은 미확인(Xcode 27 환경에 iOS 16 런타임 없음).
...
- [x] **Step 0: 스택 제거 후 리셋 현상을 다시 재현한다 (결정 지점)**
...
| 글자 열 너비 | 5/5 | 5/5 |
| 한 손 키보드 너비 | 5/5 | 5/5 |
| 길게 누르기 입력 (지연 시간 / 반복 속도) | 5/5, 5/5 | 5/5, 5/5 |
| 커서 이동 (활성화 거리 / 이동 간격) | 5/5, 5/5 | 5/5, 5/5 |

**실행 결과 (2026-09-23, 빌드 `1b384989`, iPhone Air / iOS 27)**: 스택 제거 후에도 전부 재현된다. 중첩 `NavigationStack`은 원인이 아니다. Step 1로 간다.
...
사용자 결정(길게 누르기·커서 이동): Step 1의 `.id` 수정이 통하는지 확인한 뒤 Step 3 전에 물었다(수정이 통하지 않으면 질문이 필요 없으므로 순서만 바꿨다). 2026-09-23 답: **포함해서 수정**.
...
- [x] **Step 3 (Step 0에서 사용자가 승인한 경우만): 길게 누르기 입력·커서 이동 화면에 같은 수정을 넣는다**
...
|---|---|---|---|
| 길게 누르기 지연 시간 | 0/5 | 따라옴 (0.75) | 기본값 |
| 키 반복 속도 | 0/5 | 따라옴 (0.75) | 기본값 |
| 활성화 드래그 거리 | 0/5 | 따라옴 (0.77) | 기본값 |
| 이동 드래그 간격 | 0/5 | 따라옴 (0.75) | 기본값 |

결과 (2026-09-23, iPhone Air / iOS 27): 빌드 → `exit=0`, `** BUILD SUCCEEDED **`. 행 '리셋' 좌표는 (358, 226)·(358, 336). `slider_reset_repro.py`와 `row_after_reset.py`(scratchpad `issue-151/`, 끝까지 민 직후 리셋 → 곧바로 3/4 드래그, 끝까지 민 직후 리셋 → 나갔다 재진입) 모두 PASS. 마지막 확인이 기본값으로 끝나 저장값도 기본값이다.
...
- [x] **Step 2: 실기기 확인을 사용자에게 요청하고 결과를 기록한다**
...
- ~~키보드 높이·글자 열 너비·한 손 키보드 너비: 슬라이더를 끝까지 민 직후 '리셋' 한 번으로 기본값이 유지된다~~ (슬라이더 수정을 되돌려 확인 대상에서 뺐다. 이 현상은 그대로 남는 것이 맞다)
- 6개 화면의 제목·툴바·뒤로 가기, 클립보드 편집 모드·하단 바·시트가 전과 같다

실기기 결과 (2026-09-23, 사용자 확인, iPhone 15 Pro Max / iOS 27, 빌드 `667b8c1a`): 아래 세 묶음 모두 전과 같다고 확인했다.

1. 키보드 높이·글자 열 너비·한 손 키보드 너비: 툴바(취소·리셋·저장, 뒤로 버튼 없음), 배치, 미리보기, 취소·저장, 가장자리 스와이프로 뒤로 가지 않음
2. 길게 누르기 입력·커서 이동: 뒤로 버튼·제목, 배치, 슬라이더 조작 시 키보드 내림, 뒤로 버튼·스와이프로 뒤로 감, 설정 반영
3. 클립보드 기록 관리: 진입 화면, 편집 모드·'n개 선택', 하단 바 전체 선택·고정·삭제 확인 창, 행 스와이프, 상세·추가 시트, 편집 중 뒤로 가도 메인 화면에 하단 바가 남지 않음, 스와이프로 뒤로 감

별도 관찰: iPhone 13 mini / iOS 18.6 시뮬레이터에서 클립보드 화면 상단 '+'와 '선택' 사이가 넓어 보인다. 변경 전 앱(`before.app`) 캡처와 픽셀이 같아 이번 브랜치의 회귀가 아니다. 이번 범위 밖이며 따로 다룰지는 사용자가 정한다.
...
이후 통합(PR 생성 등)은 superpowers:finishing-a-development-branch로 사용자와 정한다. PR 제목은 슬라이더 수정을 되돌렸으므로 `Refactor/#151 설정 화면 중첩 NavigationStack 제거` 형식을 쓰고, 본문에 슬라이더 현상은 iOS 27 버그로 판단해 수정하지 않았다고 적는다.

---

## 되돌리는 법

- 리셋 수정은 이미 `revert: #151` 커밋(`09ebe9b3`)으로 되돌렸다. 다시 넣으려면 그 커밋의 코드 부분을 되돌린다. 화면마다 카운터 선언, `.id(...)`, `+= 1` 세 곳뿐이다.
- 스택 제거 되돌리기: Task 2·3의 `refactor:` 커밋(`cb9f6b34`, `e7faa99c`, `1b384989`)을 `git revert`한다. 슬라이더 수정과 되돌림이 서로 상쇄돼 코드가 스택 제거 직후와 같으므로 충돌하지 않는다.
