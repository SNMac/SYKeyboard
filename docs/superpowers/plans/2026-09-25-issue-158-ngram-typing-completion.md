...
위 시뮬레이터 확인 결과와 아래 실기기 성능 측정(항목 9)을 기록했다. 실기기(iPhone 15 Pro Max / iOS 27.0)에서 확인한 것은 항목 9의 `NGramCompletions` 측정과 입력 지연 체감(느리게 느껴지지 않음, Task 9 Step 4)이다. **항목 1~8의 실기기 확인은 하지 않았다.** 특히 항목 8(입력 중 완성 후보를 길게 눌러 삭제한 뒤 사라지는지)은 시뮬레이터에서도 확인창까지만 봤으므로 미확인이다. PR 검증 항목에 같은 내용을 남긴다.
