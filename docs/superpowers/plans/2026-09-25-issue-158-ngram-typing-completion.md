...
위 시뮬레이터 확인 결과와 아래 실기기 성능 측정(항목 9)을 기록했다. 실기기(iPhone 15 Pro Max / iOS 27.0)에서 확인한 것은 항목 9의 `NGramCompletions` 측정과 입력 지연 체감(느리게 느껴지지 않음, Task 9 Step 4)이다. 항목 8(입력 중 완성 후보를 길게 눌러 삭제)은 PR 작성 뒤 사용자가 실기기에서 길게 눌러 삭제되는 것을 확인했다(2026-09-27, 사용자 보고). 항목 1~7은 사용자 결정에 따라 위 시뮬레이터 확인으로 실기기 확인을 대신한다. 시뮬레이터에서 빠진 항목 5의 '오늘 SY키'는 `SuggestionControllerNGramCompletionTests`의 `test한글과영문이섞인단어는_단어전체를_완성후보로대치`가 앞 단어 문맥과 대치 결과를 컨트롤러 경로로 확인한다.
