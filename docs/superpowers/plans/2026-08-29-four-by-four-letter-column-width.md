...
**미해결 1건 — 한영 통합 키보드 (항목 `한영 통합 키보드에서 한영 전환 버튼과 지구본 버튼의 폭`)**

`NumericKeyboardView` 실측(375pt) 결과, 하단 스페이스 **끔** + 배율 **1.15** + 지구본 **숨김**
조건에서 `languageSwitchButton`이 이론값 20.7pt가 아니라 **6.0pt로 붕괴**하고 내부 서브뷰
프레임이 `1.797e308`로 오염된다. 사용자가 "한/영 버튼이 안 보인다"고 보고한 상태다.

원인은 `switchButton`이 라벨 때문에 약 45.7pt 아래로 압축되지 않는 것이다. 열이 51.7pt로
줄면 한/영에게 6pt만 남고 우선순위 999인 폭 제약이 양보하면서 깨진다. 배율 1.0에서는
열이 93.7pt라 한/영 37.3pt로 정상이다. **이번 변경이 드러낸 회귀다.**

상한을 낮춰서는 해결되지 않는다. `switchButton` 45.7pt를 확보하려면 열이 76pt 이상이어야
하고 375pt 기준 배율 1.06 이하를 뜻한다. 한 손 키보드에서는 더 낮다.
...
하단 스페이스 **켬**에서는 modifier 스택이 넓어지는 1열에 오므로 배율 1.15에서도 정상이다
(열 107.7, 한/영 43.0).
