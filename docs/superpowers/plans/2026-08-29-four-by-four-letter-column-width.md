...
- [x] 실제 입력 앱에서 기본값(100)이 기존 배치와 동일한가
- [x] 배율을 올렸을 때 나랏글·천지인·숫자 키패드의 실제 화면
- [x] 한 손 키보드 모드에서의 레이아웃
- [x] 가로 모드에서의 레이아웃
- [x] 상한(115)에서 삭제·스페이스·리턴·전환 버튼의 터치 정확도
- [x] 좁은 기능 열에서 `!#1` 드래그 시 키보드 선택 오버레이 취소 영역 동작
- [x] 설정 화면 미리보기에서 `!#1`을 드래그해 숫자 키패드로 전환한 뒤 슬라이더가 반영되는지
- [ ] 한영 통합 키보드에서 한영 전환 버튼과 지구본 버튼의 폭
- [x] 슬라이더가 상한 115까지 도달하고, 저장 후 재진입 시 115가 유지되며, 리셋이 100으로 돌아가는가
- [x] 노치 또는 Dynamic Island 기기에서 Auto Layout 경고 여부

### 수동 확인 기록

**확인됨 (iPhone SE, iPhone 13 mini, iOS 16.0)**

사용자가 실기기에서 확인했다. 배율 100/115, 세로·가로, 한 손 모드, 오버레이 드래그,
설정 미리보기, 슬라이더 상한 도달과 저장 왕복까지 포함한다. Auto Layout 경고는 없었다.
아래 "기존 문제"로 분류한 3건을 제외하면 이상 없음.

**수정 완료, 실기 재확인 미완 — 한영 통합 키보드 (항목 `한영 통합 키보드에서 한영 전환 버튼과 지구본 버튼의 폭`)**

실기기 확인에서 발견된 붕괴는 **코드상 수정이 끝났다**(아래 "통합 키보드 modifier 균등 분배
전환" 절 참고). 체크박스가 미체크인 이유는 **수정 후 실기기 재확인을 아직 하지 않았기
때문이다.** 자동 테스트(붕괴 회귀 2건)는 시뮬레이터 프레임 계산이므로 `1.797e308` 프레임
오염이 재발하지 않는다는 실기 관찰을 대체하지 않는다.

재확인할 것:

- 한영 통합 키보드 + 하단 스페이스 끔 + 배율 115에서 한/영 버튼이 보이는지.
  균등 분배이므로 한/영과 `!#1`이 같은 폭이어야 한다(375pt 기준 각 25.8pt).
- 지구본 켬/끔 양쪽. 켜면 세 버튼이 3등분, 끄면 두 버튼이 2등분.
- **배율 100에서** 한/영 37.5 → 46.9pt, `!#1` 56.3 → 46.9pt로 두 버튼이 같은 폭이 되는
  변화. 사용자가 승인한 무회귀 계약의 유일한 예외이며, 실물로 보고 판단할 항목이다.

발견 당시 실측 (375pt, 하단 스페이스 끔) — 수정 전 상태:

| 배율 | 지구본 | modifier 열 | 한/영 | 전환 |
| --- | --- | --- | --- | --- |
| 1.00 | 숨김 | 93.7 | 37.3 | 56.3 |
| 1.00 | 표시 | 93.7 | 31.0 | 31.3 |
| 1.15 | 숨김 | 51.7 | **6.0 (붕괴)** | 45.7 |
| 1.15 | 표시 | 51.7 | 17.0 | 17.3 |

원인은 `switchButton`이 라벨 때문에 약 45.7pt 아래로 압축되지 않는 것이었다. modifier 열이
51.7pt로 줄면 한/영에게 6pt만 남고 우선순위 999인 폭 제약이 양보하면서 깨졌다. 상한을
낮춰서는 해결되지 않았다. 전환 버튼 45.7pt를 확보하려면 열이 76pt 이상이어야 하고 375pt
기준 배율 1.06 이하를 뜻한다.

**이번 변경과 무관한 기존 문제 3건**

`git log --oneline d2a99271..HEAD -- <파일>`로 이번 브랜치가 해당 파일을 건드리지 않았음을
확인했다. 별도 이슈로 분리한다.

1. **가로 모드에서 버튼 힌트 라벨이 본문과 겹친다.** `한/A`의 사선, `!#1`의 `123▷`·`⌨︎△`가
   가운데 라벨과 겹친다. 힌트 라벨이 버튼 모서리에 고정 오프셋으로 붙어 있는데 가로 모드는
   행 높이가 낮다(`landscapeKeyboardHeight` 188pt 기준 행당 약 35pt, 세로는 약 48pt).
   폭이 아니라 높이 문제이며 사용자가 배율 100에서도 동일함을 확인했다.
2. **`한/A` 버튼의 사선 각도가 가로·세로에서 다르다.** 가로에서도 세로처럼 보이게 해달라는
   요청이 있다.
3. **회전 중 Auto Layout 경고.** 한 손 키보드 최소 너비 제약(`UIView.width >= 300`,
   `KeyboardView.swift:197`의 `keyboardLayoutWidthConstraint`)이 회전 도중 일시적으로 좁아진
   폭(`UIView-Encapsulated-Layout-Width == 225`)과 충돌한다. 충돌 목록에 이번 변경의 열 폭
   제약은 하나도 없다. 회전이 끝나면 해소된다.


**확인됨 (상한 1.20 빌드, iPhone SE)**

- **기본값 100에서 기존 배치와 동일함을 사용자가 실기기에서 확인했다.** 이 기능의 핵심
  무회귀 계약이다. `r = 1.0`의 결과는 이후 상한 조정(1.20 → 1.15)과 무관하므로 이 확인은
  그대로 유효하다.
- 배율을 올린 상태에서도 시각적으로 이상 없음. 다만 세 키보드와 두 배치를 모두 봤는지는
  확인하지 않았으므로 위 두 번째 항목은 미확인으로 남긴다.
- iPhone SE에서 Auto Layout 경고 없음.

**이 확인이 덮지 못하는 범위**

- iPhone SE는 홈 버튼 기기라 safe area 하단 inset이 0이고 렌더링이 2x다. 노치·Dynamic Island
  기기는 34pt 홈 인디케이터 위에 키보드가 앉고 대부분 3x다. 폭이 아닌 이 두 축은 확인되지
  않았다. 폭이 SE와 같은 375pt인 iPhone 13 mini로 보면 변수를 하나만 바꿔 확인할 수 있다.
- 추가한 제약은 전부 비율 기반이라 폭에 따라 뒤집히는 임계값이 없다. 따라서 폭이 다른 기기에서
  새 충돌이 생길 구조적 이유는 없지만, 이는 추론이지 검증이 아니다.
- 상한 1.15 빌드는 아직 확인하지 않았다. 1.20에서 시각적으로 문제가 없었다면 1.15는 기능 열이
  더 넓으므로(375pt 기준 37.5pt → 51.6pt) 이 방향의 추론은 안전하다.

**터치 타깃 실측 (기능 열 = 삭제·스페이스·리턴)**

| 배율 | 375pt | 320pt(한 손 기본) | 300pt(한 손 최소) | 지구본·전환 각각(320pt) |
| --- | --- | --- | --- | --- |
| 100 | 93.8pt | 80.0pt | 75.0pt | 40.0pt |
| 110 | 65.6pt | 56.0pt | 52.5pt | 28.0pt |
| 115 | 51.6pt | 44.0pt | 41.3pt | 22.0pt |

상한을 1.15로 정한 근거는 한 손 기본 폭에서 기능 열이 HIG 최소 44pt를 유지한다는 것이다.
**다만 나랏글에서 지구본이 표시되면 지구본과 전환 버튼이 기능 열을 반씩 나눠 가지므로 22pt가
된다.** 기본값에서도 40pt로 이미 44pt 미만이었으므로 이번 변경으로 생긴 문제는 아니지만,
체감이 나쁘면 상한을 1.10으로 한 번 더 낮추는 선택지가 있다.
...
## 최종 리뷰 fix wave (2026-08-29)

브랜치 전체 리뷰에서 Task 경계를 넘는 지적이 나와 반영했다.

- **한영 전환 버튼 폭 기준 변경**: 전체 폭이 아니라 버튼이 속한 열 폭의 40%로 건다.
  하단 스페이스 배치에서는 modifier 스택이 1열(배율을 올리면 넓어지는 열)에 오는데, 전체 폭
  기준이면 열은 넓어지고 버튼만 좁아지는 역전이 생겼다. 기본 배율 결과는 `0.4 × 0.25W = 0.1W`로
  동일해 무회귀 계약은 유지된다.
- **`KeyboardColumnWidthPolicy.languageSwitchButtonRatio` 제거**: 위 변경으로 production에서
  미사용이 되었다. 불변식은 상수 관계를 직접 고정하는 테스트로 옮겼다.
- **`update()` early-out 추가**: `updateUIViewController`가 SwiftUI 갱신마다 호출되므로
  배율이 그대로면 제약을 다시 만들지 않는다.
- **행 간격 0 의존 주석 추가**: 무회귀 계약이 `KeyboardRowHStackView.spacing == 0`에
  걸려 있다는 사실을 코드에 남겼다.
- **테스트 2개 추가**: 하단 스페이스에서 한영 전환 버튼이 자기 열에 비례하는지,
  프로토콜 디스패치로 호출해도 배율이 적용되는지.

병합 후 별도 처리로 남긴 것: 4열이 숨겨지는 배치가 생기면 `.fill`에서 required 제약이 충돌할 수
있다(현재 세 뷰에서는 도달 불가), 설정 화면 미리보기가 두벌식 사용자에게는 `!#1` 드래그 전까지
변화를 보여주지 않는다.

## 범위 조정과 슬라이더 버그 수정 (2026-08-29)

사용자 확인 결과 두 가지를 고쳤다.

- **슬라이더가 상한에 도달하지 못하는 버그**: `Slider(in:step: 0.01)`은 `0.01`이 이진
  부동소수점으로 정확하지 않아 스텝 수가 잘린다. `(1.2 − 1.0) / 0.01 = 19.999999999999996`이
  19로 잘려 최대 `1.19`(표시 119)에서 멈췄다. 슬라이더를 정수 스텝(`100...115`, step `1`)으로
  바꾸고 저장할 때 100으로 나눈다. `(115 − 100) / 1 = 15.0`이라 오차가 없다.
  `KeyboardLayoutFigure.letterColumnWidthPercentRange`를 배율 범위에서 유도해 단일 출처를
  유지한다.
- **상한을 `1.20`에서 `1.15`로**: 상한 1.15면 한 손 키보드 기본 폭(320pt)에서 기능 열이
  정확히 44pt로 Apple HIG 최소 터치 타깃을 유지한다. 1.20에서는 32pt까지 내려갔다.
  하한과 기본값은 `1.0` 그대로이며 늘리는 방향만 조절한다.

테스트의 최대 배율 기대값을 `0.3`/`0.1`/`0.9`에서 `0.2875`/`0.1375`/`0.8625`로 갱신했고,
정수 범위와 배율 범위가 일치하는지 검증하는 테스트를 추가했다.

## 통합 키보드 modifier 균등 분배 전환 (2026-08-29)

실기기 확인에서 한영 통합 키보드의 4행 modifier 스택이 배율을 올리면 깨지는 것이 드러나
설계를 바꿨다.

- **증상**: 하단 스페이스 끔 + 배율 1.15 + 지구본 숨김에서 `languageSwitchButton`이 6.0pt로
  붕괴하고 내부 서브뷰 프레임이 `1.797e308`로 오염됐다. 사용자에게는 한/영 버튼이 보이지
  않았다.
- **원인**: `switchButton`이 라벨 때문에 약 45.7pt 아래로 압축되지 않는다. modifier 열이
  51.7pt로 줄면 한/영에게 6pt만 남고 우선순위 999인 폭 제약이 양보하면서 깨진다.
  상한을 낮춰서는 해결되지 않는다. 전환 버튼 45.7pt를 확보하려면 열이 76pt 이상이어야 하고
  375pt 기준 배율 1.06 이하를 뜻한다.
- **해결**: modifier 스택을 항상 균등 분배(`.fillEqually`)로 바꾸고 한영 전환 버튼의 비율 폭
  제약을 제거했다. 균등 분배는 required 등폭 제약이라 해가 항상 결정되어 붕괴하지 않는다.
- **수용한 대가**: 배율 1.0에서 한영 통합 키보드의 modifier 버튼 폭이 바뀐다.
  375pt·지구본 숨김 기준 한/영 37.5 → 46.9pt, `!#1` 56.3 → 46.9pt. 무회귀 계약의 유일한
