...
- [x] 한영 통합 키보드에서 한영 전환 버튼과 지구본 버튼의 폭
...
**수정 완료, 실기 재확인 완료 — 한영 통합 키보드 (항목 `한영 통합 키보드에서 한영 전환 버튼과 지구본 버튼의 폭`)**

실기기 확인에서 발견된 붕괴를 균등 분배로 수정하고(아래 "통합 키보드 modifier 균등 분배
전환" 절 참고), 사용자가 iPhone SE·iPhone 13 mini(iOS 16.0)에서 재확인했다.

| 재확인 항목 | 결과 |
| --- | --- |
| 배율 115 + 하단 스페이스 끔에서 한/영 버튼 표시 (수정 전 6pt 붕괴) | 두 기기 모두 정상 표시 |
| 지구본 켬/끔 양쪽 | SE(지구본 표시), 13 mini(지구본 없음) 모두 정상 |
| 배율 100에서 한/영과 `!#1`이 같은 폭(46.9pt)이 되는 변화 | 시각적으로 문제 없음 |

세 번째 항목은 사용자가 승인한 무회귀 계약의 유일한 예외이며, 실물 확인으로 수용되었다.
