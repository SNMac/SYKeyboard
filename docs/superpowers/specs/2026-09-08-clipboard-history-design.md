...
| 다중·전체 삭제 | 헤더의 "선택"으로 편집 모드 진입. 시스템 다중 선택 컨트롤(체크마크)을 쓰고 "전체 선택"·"n개 삭제"·"완료" 버튼을 둔다. 확인 알림은 없다. |
...
- 헤더 줄(높이 고정): 평소에는 왼쪽 "클립보드 기록" 라벨, 오른쪽 "선택" 버튼.
  편집 모드에서는 왼쪽 "전체 선택"(모두 선택되면 "선택 해제". 글자 수가 같아 버튼 폭이 변하지
  않는다), 오른쪽 "n개 삭제"와
...
- 빈 상태 라벨 "복사한 텍스트가 여기에 표시됩니다", Full Access 안내 라벨
  "전체 접근 허용이 필요합니다". 두 상태에서는 헤더의 "선택"을 숨긴다.
...
- 삭제 대상(스와이프·편집 모드)에 고정 항목이 있으면 델리게이트를 바로 부르지 않고 패널을 덮는
  확인 뷰(`ClipboardHistoryDeleteConfirmView`, 블러 + 제목 + "취소"/"삭제")를
  크로스페이드로 띄운다. "삭제"를 누르면 그때 델리게이트를 부르고, "취소"나 `configure`·
  `resetPresentation`은 확인을 버린다. 스와이프 액션의 completion에는 확인 대기면 `false`를 넘겨
  행이 그대로 남게 한다. `resetPresentation`은 열린 스와이프 액션도 닫는다. 삭제는 델리게이트가
  받은 인덱스를 패널 목록의 텍스트로 바꿔 `store.remove(texts:)`로 지워, 확인을 기다리는 사이
  파일 순서가 바뀌어도 다른 항목을 지우지 않는다. 제목은 전부 고정이면 "고정 항목 n개를 삭제할까요?",
  미고정이 섞였으면 "항목 n개를 삭제할까요?"이고 설명에 "고정 항목 m개가 포함되어 있습니다."를
  덧붙인다(앱 시트와 같은 문구). 키보드 extension은 시스템 알림을 띄울 수 없어 패널 안에서
...
- 키보드 패널과 같은 목록(고정 최신순 → 미고정 최신순), 고정 행은 `pin.circle.fill`.
  행 탭은 원문 전체를 보는 상세 화면이다(붙여넣기는 없다). 행은 `Button`이 아니라 탭 제스처로
  시트를 연다. `Button`이면 시트를 띄우는 탭 뒤에 눌린 표시가 남는 일이 있었다(실기기).
...
- 삭제 대상에 고정 항목이 있으면(스와이프·편집 모드 모두) `confirmationDialog`(action sheet)로
  확인받고, 미고정만이면 바로 지운다. 문구는 키보드 확인 뷰와 같다(전부 고정 "고정 항목 n개를
  삭제할까요?", 섞이면 "항목 n개를 삭제할까요?" + 고정 개수 설명). 시트는 요청한 곳에 붙인다.
  스와이프 삭제는 그 행, 편집 모드 삭제는 하단 바의 삭제 버튼이라, 시트를 출처에 붙여 띄우는 OS
  (iOS 26+)에서는 그 근처에서 뜨고 iOS 16에서는 하단 시트로 뜬다. HIG가 "의도적으로
  시작한 동작에 딸린 선택지는 알림이 아니라 action sheet로" 제공하라고 하기 때문이다. 취소는
  시스템이 붙인다. 제목의 개수는 시트를 띄울 때 별도 상태에 담아, 닫히는 동안 "0개"로 바뀌지 않게
  한다. 삭제는 `store.remove(texts:)`라 파일을 미리 다시 읽지 않는다. 키보드 패널은 4절의 패널 안
  확인 뷰를 쓴다.
- 앱 스와이프 삭제 버튼은 `role: .destructive` 없이 `.tint(.red)`만 준다. SwiftUI `List`는
  destructive 스와이프 버튼을 누르는 순간 행 제거 애니메이션을 시작하는데, 확인 시트를 그 행에
  붙인 뒤로는 행이 접히면서 시트까지 닫혔다(iOS 27 실기기 녹화). `UITableView` 전환은 iOS 26+에서
  지워지는 행이 카드 위로 올라가는 다른 문제가 있어(세 가지 수정 시도 무효) `List`를 유지한다.
  상단 툴바는 `+`(추가) 오른쪽에 "선택"/"완료"를 두고, "완료"는 semibold다. 두 문구 중 넓은
...
- 복사 버튼 오른쪽의 "편집"(`pencil.line`)을 누르면 같은 시트가 `TextEditor`로 바뀌고 "취소"/"저장"만
  ...
  (빈 값·2,000자 초과·원문과 같음이면 비활성) 자리·고정 상태·시각은 유지된다. 다른 항목과 같은
  내용으로 저장하면 편집한 항목을 지우고 기존 항목 하나만 남긴다. 둘 중 하나라도 고정이었으면 남는
  항목은 지금 고정한 것처럼 고정 맨 위로, 둘 다 미고정이면 최근 복사한 것처럼 미고정 맨 위로 온다.
  `+`로 이미 고정된 텍스트를 추가할 때도 그 항목이 고정 맨 위로 온다(`+`는 고정 한도가 차면
  비활성이므로 한도에 여유가 있을 때의 동작이다. 정책 함수 자체는 한도를 보지 않는다). 복사·붙여넣기는 이전과 같이
  고정 항목의 자리를 바꾸지 않는다.
