...
- 삭제 대상(스와이프·편집 모드)에 고정 항목이 있으면 델리게이트를 바로 부르지 않고 패널을 덮는
  ...
  파일 순서가 바뀌어도 다른 항목을 지우지 않는다. 키보드 extension은 시스템 알림을 띄울 수 없어 패널 안에서
...
- 삭제 대상에 고정 항목이 있으면(스와이프·편집 모드 모두) "고정 항목 n개를 삭제할까요?"를
  `confirmationDialog`(action sheet)로 확인받고, 미고정만이면 바로 지운다. HIG가 "의도적으로
