...
- 삭제 대상에 고정 항목이 있으면(스와이프·편집 모드 모두) `confirmationDialog`(action sheet)로
  ...
  확인 뷰를 쓴다.
- 알려진 제한: SwiftUI `List`는 destructive 스와이프 버튼을 누르는 순간 행 제거 애니메이션을
  시작해, 고정 항목의 확인 시트가 뜨는 동안 행이 접혔다 돌아온다. `UITableView`로 바꾸면 이건
  해결되지만 iOS 26+에서 지워지는 행이 카드 위로 올라가는 다른 문제가 생겨(실기기 녹화, 세 가지
  수정 시도 무효) `List`를 유지한다. role을 빼고 `tint`만 주는 방법은 우회로 보고 쓰지 않는다.
