...
| 다중·전체 삭제 | 헤더의 "편집"으로 편집 모드 진입. 시스템 다중 선택 컨트롤(체크마크)을 쓰고 "전체 선택"·"삭제 (n)"·"완료" 버튼을 둔다. 확인 알림은 없다. |
...
- 헤더 줄(높이 고정): 평소에는 왼쪽 "클립보드 기록" 라벨, 오른쪽 "편집" 버튼.
  편집 모드에서는 왼쪽 "전체 선택"(모두 선택되면 "선택 해제"), 오른쪽 "삭제 (n)"과
  "완료". "삭제 (n)"은 선택 0개면 비활성. 항목이 0개면 "편집"도 비활성.
...
- 상세 뷰 "붙여넣기" → 행 탭과 같은 `didSelectItemAt`. "닫기" → 상세 뷰만 닫는다.
- 편집 모드 행 탭 → 선택 토글(붙여넣기 안 함). "삭제 (n)" → 전부 선택이면
