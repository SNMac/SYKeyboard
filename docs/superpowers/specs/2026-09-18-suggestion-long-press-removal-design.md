...
자동완성 바의 추천 단어를 0.7초 동안 길게 누르면 키보드 위에 삭제 확인 오버레이를 띄운다.
...
'내일'을 0.7초 동안 누름 → 키보드 전체를 덮는 오버레이
┌──────────────────────────────┐
│ '내일'을 자동완성에서 삭제할까요? │
│ 다시 입력하면 다시 학습됩니다.     │
│      [ 취소 ]  [ 삭제 ]         │
...
- 0.7초를 쓰는 이유: iOS 기본값 0.5초는 손가락을 댄 채 옆 후보로 옮겨 고르는 드래그 선택 중에 넘기기 쉽다.
...
- [ ] NGram 후보를 0.7초 누르면 오버레이 표시, 손을 떼도 후보가 입력되지 않음
