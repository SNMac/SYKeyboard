...
- [ ] `SuggestionController` 엔진 생성 타이밍을 측정한다.
...
- [ ] 첫 표시 전 필수 초기화와 지연 가능한 초기화를 확정한다.
  - 완료 기준: 변경할 경로와 그대로 둘 경로가 plan/context에 기록된다.
- [ ] `SuggestionController`에서 설정 저장과 엔진 생성 시점을 분리하는 테스트 또는 기존 테스트 영향 범위를 정한다.
