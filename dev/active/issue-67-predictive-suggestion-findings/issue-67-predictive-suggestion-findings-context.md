...
- 추정: 커서 이동 시 `textWillChange(_:)`에서 `inputBuffer`가 초기화된 뒤 `textDidChange(_:)`가 호출되므로, `updateSuggestions()`가 빈 `inputBuffer`를 기준으로 실행되어 n-gram 초기 후보로 돌아갈 수 있다.
- 추정: `textDocumentProxy.documentContextBeforeInput`에서 마지막 단어를 추출해 자동완성 조회용 base text로 사용하면 커서 앞 글자에 맞는 후보를 다시 표시할 수 있다.
