...
- [x] 텍스트 대치 suffix 오작동을 재현하는 실패 테스트를 추가한다.
- [x] 텍스트 대치가 현재 단어와 단축어의 정확한 일치만 허용하도록 수정한다.
- [x] 대치 복구 이력이 다른 위치의 동일 문구를 복구하지 않는 실패 테스트를 추가한다.
- [x] 대치 직후 삭제 복구는 유지하면서 커서/focus/context 변경 후 복구 이력을 무효화한다.
- [x] lexicon 로딩 전 첫 텍스트 대치 누락 정책을 테스트 가능한 단위로 확정한다.
- [x] 확정한 정책에 따라 lexicon 준비/재평가 경로를 수정한다.
- [x] n-gram background load/save/reset race를 재현하는 테스트를 추가한다.
...
- [x] `dev/active/code-review-scope/code-review-scope-findings.md`와 이 작업 문서에 처리/검증 결과를 반영한다.
