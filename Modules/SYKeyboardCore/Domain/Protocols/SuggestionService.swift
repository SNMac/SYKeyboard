...
///
/// ## 채택 구현체
/// - `SuggestionController`: `UILexicon` + `UITextChecker` + Foundation Models를 조합한 기본 구현
///
/// ## 동작 흐름
/// 1. **입력 중**: `updateSuggestions(contextBeforeInput:)`로 후보 갱신
/// 2. **후보 탭**: `selectSuggestion(at:contextBeforeInput:)`로 현재 단어 교체
/// 3. **스페이스**: `attemptTextReplacement(contextBeforeInput:)`로 텍스트 대치 수행
/// 4. **삭제**: `attemptRestoreReplacement(contextBeforeInput:)`로 대치 복구
/// 5. **기타 키 입력**: `clearIgnoredShortcut()`으로 재대치 방지 상태 초기화
protocol SuggestionService: AnyObject {
    ...
    ///
    /// 갱신 결과는 `delegate`의
    /// `SuggestionControllerDelegate/suggestionController(_:didUpdateSuggestions:)`를 통해 전달됩니다.
    /// 마지막 문자가 공백이거나 `isEnabled`가 `false`이면 갱신을 건너뜁니다.
    ///
