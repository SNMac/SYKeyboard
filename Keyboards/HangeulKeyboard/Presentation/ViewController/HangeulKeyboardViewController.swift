...
final class HangeulKeyboardViewController: BaseKeyboardViewController {
    ...
    override func getInputText(from keys: [String]) -> String {
        ...
        let beforeText = if let beforeContext = self.textDocumentProxy.documentContextBeforeInput {
            String(beforeContext.reversed().prefix(while: { !$0.isWhitespace }).reversed())
        } else {
            ""
        }
